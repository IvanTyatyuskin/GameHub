...
function LobbyList(props) {
    ...
    const lobbyName = lobbyNameInput.value;
        if (message.trim() !== '') {
            socket.emit('chat message', { lobbyName, currentLobby, message, nickname });
            chatInput.value = '';
        }
    }
    ...
    return(
       ...
       <div>
        ...
        <form id="lobby-form">
          ...
          <button type="button" id="leave-lobby" className="DisplayText" onClick={LeaveLobby()}>Leave lobby</button>
        </form>
      
        <div id="lobby-chat" >
          <h2>Lobby Chat</h2>
          <div id="chat-messages"></div>
          <form id="chat-form" onSubmit={ChatMessage()}>
